...
public class Telemetry {
  ...
  // private static MechanismLigament2d intakePivot = intakeRoot.append(new
  // MechanismLigament2d("Intake", 15, 0));
  private static MechanismRoot2d shooterRoot = subsystems.getRoot("Shooter", 20, 10);
  private static MechanismLigament2d shooterPivot =
      shooterRoot.append(new MechanismLigament2d("Shooter", 10, 180));
  ...
  private static MechanismLigament2d fieldSwerveMod0 =
      fieldSwerveMod0Root.append(new MechanismLigament2d("fieldSwerveMod0", 0, 0));
  private static MechanismRoot2d fieldSwerveMod1Root = customField.getRoot("fieldSwerveMod1", 0, 0);
  private static MechanismLigament2d fieldSwerveMod1 =
      fieldSwerveMod1Root.append(new MechanismLigament2d("fieldSwerveMod1", 0, 0));
  private static MechanismRoot2d fieldSwerveMod2Root = customField.getRoot("fieldSwerveMod2", 0, 0);
  private static MechanismLigament2d fieldSwerveMod2 =
      fieldSwerveMod2Root.append(new MechanismLigament2d("fieldSwerveMod2", 0, 0));
  private static MechanismRoot2d fieldSwerveMod3Root = customField.getRoot("fieldSwerveMod3", 0, 0);
  private static MechanismLigament2d fieldSwerveMod3 =
      fieldSwerveMod3Root.append(new MechanismLigament2d("fieldSwerveMod3", 0, 0));
  private static MechanismRoot2d fieldSwerveMovementRoot =
      customField.getRoot("Field Movement Vector", 0, 0);
  private static MechanismLigament2d fieldSwerveMovement =
      fieldSwerveMovementRoot.append(new MechanismLigament2d("Field Movement Vector", 0, 0));
  ...
  private static MechanismLigament2d swerveMod0 =
      swerveMod0Root.append(new MechanismLigament2d("SwerveMod0", 0, 0));
  private static MechanismRoot2d swerveMod1Root = swerveMods.getRoot("SwerveMod1", 0, 0);
  private static MechanismLigament2d swerveMod1 =
      swerveMod1Root.append(new MechanismLigament2d("SwerveMod1", 0, 0));
  private static MechanismRoot2d swerveMod2Root = swerveMods.getRoot("SwerveMod2", 0, 0);
  private static MechanismLigament2d swerveMod2 =
      swerveMod2Root.append(new MechanismLigament2d("SwerveMod2", 0, 0));
  private static MechanismRoot2d swerveMod3Root = swerveMods.getRoot("SwerveMod3", 0, 0);
  private static MechanismLigament2d swerveMod3 =
      swerveMod3Root.append(new MechanismLigament2d("SwerveMod3", 0, 0));
  private static MechanismRoot2d swerveMovementRoot = swerveMods.getRoot("Movement Vector", 0, 0);
  private static MechanismLigament2d swerveMovement =
      swerveMovementRoot.append(new MechanismLigament2d("Movement Vector", 0, 0));
  ...
  private static final GenericEntry moveDeadband =
      moveTab.add("Move Deadband", SwervePID.moveDead).getEntry();
  ...
  private static final GenericEntry rotDeadBand =
      rotTab.add("Rot Deadband", SwervePID.rotDead).getEntry();

  // Shooter
  private static final GenericEntry TOPFLYWHEELRPM =
      shooter.add("Top Flywheel RPM", Shooter.topRPM).getEntry();
  private static final GenericEntry BOTTOMFLYWHEELRPM =
      shooter.add("Bottom Flywheel RPM", Shooter.bottomRPM).getEntry();
  private static final GenericEntry FLYWHEELTARGETRPM =
      shooter
          .add("Flywheel Targeted RPM", Shooter.targetVelocity * ShooterConstants.VelToRPM)
          .getEntry();
  private static final GenericEntry TOPVECTOR =
      shooter.add("Top Flywheel Vector", OI.topVector).getEntry();
  private static final GenericEntry BOTTOMVECTOR =
      shooter.add("Bottom Flywheel Vector", OI.bottomVector).getEntry();
  // private static final GenericEntry FLYWHEELATVEL = shooter.add("Flywheel At Velocity",
  // Shooter.canShoot()).getEntry();

  // Climber
  private static final GenericEntry LEFTCLIMBERSTATE =
      climber
          .add("Left Climber State", ClimberManager.leftClimber.climberControlState.name())
          .getEntry();
  private static final GenericEntry RIGHTCLIMBERSTATE =
      climber
          .add("Right Climber State", ClimberManager.rightClimber.climberControlState.name())
          .getEntry();

  private static final GenericEntry pivotAngle =
      shooter.add("Pivot Angle", ShooterPivot.actualPos).getEntry();
  private static final GenericEntry pivotTargetAngle =
      shooter.add("Pivot Target Angle", ShooterPivot.targetPos).getEntry();

  // SwervePosition
  private static final GenericEntry swervePos =
      pos.add("Swerve Position", SwervePosition.getPosition().toString()).getEntry();
  ...
  /**
   * Log a message to the console. Same as a print statement, however this has color, and it is
  ...
  /**
   * Log a message to the console. Same as a print statement, however this has color, and it is
   * prefixed with a timestamp. The name of the class that called this function is automatically
   * detected and printed as well.
   *
   * @param severity The severity of the message as a string. This changes the color of the message.
   * @param message The message to send to the console.
   */
   ...
   *     This is just so we don't hog network bandwidth, and it still gives us decently important
   *     information.
   */
  public static void update(boolean compMode) {
    swervePublisher.set(SwerveManager.getStates());

    // -1 if we're on the red alliance, 1 if we're on the blue alliances
    ...
    if (RobotBase.isSimulation()) {
      ...
      Vector2 modifiedSimPos =
          new frc.robot.utils.Vector2(field.getRobotPose().getX(), field.getRobotPose().getY());
      if (prevSimPos.sub(modifiedSimPos).mag() > 0.001) {
        Vector2 modifiedSwervePos = modifiedSimPos.div(IN_TO_M).sub(new Vector2(325.62, 157.75));
        SwervePosition.setPosition(
            new Vector2(
                (-allianceMultiplier) * modifiedSwervePos.y,
                modifiedSwervePos.x * allianceMultiplier));
      }
    ...
    Vector2 fieldPosMeters =
        new Vector2(
                SwervePosition.getPosition().y * allianceMultiplier,
                allianceMultiplier * -SwervePosition.getPosition().x)
            .add(new Vector2(325.62, 157.75))
            .mul(IN_TO_M);

    Rotation2d rotation =
        Rotation2d.fromRadians(Pigeon.getRotationRad() - Math.PI / 2 * allianceMultiplier);
    field.setRobotPose(fieldPosMeters.x, fieldPosMeters.y, rotation);

    /*if(desiredPos != null){
        // Update desired trajectory positions (convert to meters)
        Vector2 trajPosMeters = new Vector2(desiredPos.getY() * allianceMultiplier, -desiredPos.getX())
            .add(new Vector2(325.62, 157.75))
            .mul(IN_TO_M);
    ...
    double modDist =
        Math.sqrt(
            Math.pow(Constants.Swerve.SWERVEMODX0, 2) + Math.pow(Constants.Swerve.SWERVEMODY0, 2));

    fieldSwerveMod0Root.setPosition(
        modDist * Math.cos(Pigeon.getRotationRad() + 3 * Math.PI / 4)
            + 450
            + SwervePosition.getPosition().y,
        modDist * Math.sin(Pigeon.getRotationRad() + 3 * Math.PI / 4)
            + 250
            + -SwervePosition.getPosition().x);
    fieldSwerveMod0.setAngle(
        Math.toDegrees(SwerveManager.mods[0].getSteerAngle() + Pigeon.getRotationRad()) - 180);
    fieldSwerveMod0.setLength(SwerveManager.mods[0].getDriveVelocity() / 30);

    fieldSwerveMod1Root.setPosition(
        modDist * Math.cos(Pigeon.getRotationRad() + 1 * Math.PI / 4)
            + 450
            + SwervePosition.getPosition().y,
        modDist * Math.sin(Pigeon.getRotationRad() + 1 * Math.PI / 4)
            + 250
            + -SwervePosition.getPosition().x);
    fieldSwerveMod1.setAngle(
        Math.toDegrees(SwerveManager.mods[1].getSteerAngle() + Pigeon.getRotationRad()) - 180);
    fieldSwerveMod1.setLength(SwerveManager.mods[1].getDriveVelocity() / 30);

    fieldSwerveMod2Root.setPosition(
        modDist * Math.cos(Pigeon.getRotationRad() + -1 * Math.PI / 4)
            + 450
            + SwervePosition.getPosition().y,
        modDist * Math.sin(Pigeon.getRotationRad() + -1 * Math.PI / 4)
            + 250
            + -SwervePosition.getPosition().x);
    fieldSwerveMod2.setAngle(
        Math.toDegrees(SwerveManager.mods[2].getSteerAngle() + Pigeon.getRotationRad()) - 180);
    fieldSwerveMod2.setLength(SwerveManager.mods[2].getDriveVelocity() / 30);

    fieldSwerveMod3Root.setPosition(
        modDist * Math.cos(Pigeon.getRotationRad() + -3 * Math.PI / 4)
            + 450
            + SwervePosition.getPosition().y,
        modDist * Math.sin(Pigeon.getRotationRad() + -3 * Math.PI / 4)
            + 250
            + -SwervePosition.getPosition().x);
    fieldSwerveMod3.setAngle(
        Math.toDegrees(SwerveManager.mods[3].getSteerAngle() + Pigeon.getRotationRad()) - 180);
    fieldSwerveMod3.setLength(SwerveManager.mods[3].getDriveVelocity() / 30);

    fieldSwerveMovementRoot.setPosition(
        450 + SwervePosition.getPosition().y, 250 + -SwervePosition.getPosition().x);
    fieldSwerveMovement.setAngle(
        Math.toDegrees(SwerveManager.getRobotDriveVelocity().atan2() + Pigeon.getRotationRad())
            - 180);
