...
public class SwervePID {
    ...
    if (!Robot.isSimulation()) {
      return (xPID.atSetpoint()
          ...
          : (DriverStation.getAlliance().get() == Alliance.Red ? -1 : 1)
              * xPID.updateOutput(SwervePosition.getPosition().x));
    } else {
  ...
  public static double updateOutputRot() {
    if (RobotBase.isReal()) return rotPID.updateOutput(Pigeon.getRotationRad());
    else {
  ...
  public static boolean atRot() {
    return (Robot.isSimulation()
            && Pigeon.simulatedRot > rotPID.getDest() - rotDead
            && Pigeon.simulatedRot < rotPID.getDest() + rotDead)
        || rotPID.atSetpoint();
  }
}
