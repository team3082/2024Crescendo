...
public class SwerveModule {
  ...
  private double swerveStateSpeed;
  private double swerveAngle;
  ...
  public SwerveModule(int steerID, int driveID, double cancoderOffset, double x, double y) {
    ...
    absEncoder.configSensorInitializationStrategy(
    ...
    SupplyCurrentLimitConfiguration currentLimit =
        new SupplyCurrentLimitConfiguration(true, 39, 39, 0);
    drive.configSupplyCurrentLimit(currentLimit);
    drive.configVoltageCompSaturation(11.9);
    drive.enableVoltageCompensation(true);

    SupplyCurrentLimitConfiguration steerCurrentLimit =
        new SupplyCurrentLimitConfiguration(true, 30, 30, 0);
  ...
  public SwerveModuleState getSwerveState() {
    return new SwerveModuleState(swerveStateSpeed, Rotation2d.fromDegrees(swerveAngle));
  }

  public void drive(double power) {
    drive.set(TalonFXControlMode.PercentOutput, power * (inverted ? -1.0 : 1.0));

    swerveStateSpeed = power;
  ...
  public void rotate(double toAngle) {
    swerveAngle = toAngle / 900 * 360;

    double motorPos;
    if (RobotBase.isSimulation()) motorPos = simSteerAng;
    else motorPos = steer.getSelectedSensorPosition();
    ...
    if (Math.abs(destination - motorPos) > ticksPerRotationSteer / 4.0) {
      inverted = true;
      if (destination > motorPos) destination -= ticksPerRotationSteer / 2.0;
      else destination += ticksPerRotationSteer / 2.0;
    } else {
      inverted = false;
    }
