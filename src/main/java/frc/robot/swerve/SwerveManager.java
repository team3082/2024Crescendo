...
public final class SwerveManager {
  ...
  public static SwerveModuleState[] getStates() {
    SwerveModuleState[] returnVal = new SwerveModuleState[4];
    for (int i = 3; i >= 0; i--) returnVal[i] = mods[i].getSwerveState();
    return returnVal;
  }
  ...
  public static void rotateAndDrive(double rotSpeed, Vector2 move) {
    ...
    for (int i = 0; i < mods.length; i++) {
      maxModPosMagnitude = Math.max(maxModPosMagnitude, mods[i].pos.mag());
    }
    ...
    for (int i = 0; i < mods.length; i++) {
      ...
      if (power != 0) mods[i].rotateToRad(direction);
      mods[i].drive(power);
    }
  }
  ...
  /**
   * Returns the overall drive velocity of the robot, based on the average of the velocities of the
   * wheels. Not adjusted for the rotation of the robot on the field.
   *
  ...
  /**
   * finds the robots velocity by performing pose exponentiation on each of the swerve mods, and
   * returning the average change
   *
   * @return
   */
  ...
  /**
   * Returns the overall rotational velocity of the robot, based on the rotations and velocities of
   * each of the wheels. Primarily meant for applications where the Pigeon is unavailable, such as
   * simulation.
   *
  ...
  public static SwerveModuleState[] returnStates() {
    return new SwerveModuleState[] {
      new SwerveModuleState(
          mods[0].getDriveVelocity(), Rotation2d.fromRadians(mods[0].getSteerAngle())),
      new SwerveModuleState(
          mods[1].getDriveVelocity(), Rotation2d.fromRadians(mods[1].getSteerAngle())),
      new SwerveModuleState(
          mods[2].getDriveVelocity(), Rotation2d.fromRadians(mods[2].getSteerAngle())),
      new SwerveModuleState(
          mods[3].getDriveVelocity(), Rotation2d.fromRadians(mods[3].getSteerAngle()))
    };
  }
}
