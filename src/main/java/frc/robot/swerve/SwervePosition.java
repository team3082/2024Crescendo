...
public class SwervePosition {
  ...
  // Smoothly correct field position based on vision output. VISION_CORRECTION_FACTOR should range
  // from 0.0 to
  // 1.0, representing the speed at which we blend from the odometry output to the output of the
  // vision.
  ...
  /**
   * Recalibrates the SwervePosition based on a position on the field. Should only be used when
   * vision is disabled, otherwise it'll just be overwritten the next frame.
   *
  ...
  public static Pose2d getPose() {
    return new Pose2d(
        new Translation2d(position.x, position.y), Rotation2d.fromRadians(Pigeon.getRotationRad()));
  }
}
