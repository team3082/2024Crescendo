...
public class ChoreoTrajectoryGenerator{
    ...
    private static TimedSwerveState toSwerveState(ChoreoState cs){
        return new TimedSwerveState(cs.x * METERSTOINCHES,cs.y * METERSTOINCHES,cs.heading,cs.velocityX * METERSTOINCHES,cs.velocityY * METERSTOINCHES,cs.angularVelocity, cs.timestamp);
    }
    ...
    public static DiscreteTraj generateTrajectory(String fileName){
        ...
        return new DiscreteTraj(new ArrayList<TimedSwerveState>(choreoStates.stream().map((s) -> toSwerveState(s)).toList()));
    }
